...
namespace Fbits.VueMpaTemplate.VueCliMiddleware
{
    internal static class VueCliMiddleware
    {
            ...
            string scriptName, int port = 8080, ScriptRunnerType runner = ScriptRunnerType.Npm, string regex = DefaultRegex, bool forceKill = false)
        {
            ...
            var address = addresses.Count > 0 ? addresses.First() : "";
            var portTask = StartVueCliServerAsync(sourcePath, scriptName, logger, port, runner, regex, forceKill, address);
        ...
        private static async Task<int> StartVueCliServerAsync(string sourcePath, string npmScriptName, ILogger logger, int portNumber, ScriptRunnerType runner, string regex, bool forceKill = false, string address = "")
        {
            if (portNumber < 80)
            {
